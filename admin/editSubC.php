<?php
    ...
    $cate = Operations::getSubCate($conn);
    $list = Operations::getCateChecker($conn);
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST')
    {
        ...
        if (isset($_POST['submit']) && isset($_POST['title']) && isset($_POST['category']))
        {
            ...
            $title = $_POST['title'] ?? "";
            $cate = $_POST['category'] ?? "";
            $error = User::updateSubCategory($getID, $img, $title, $cate, $conn);
        }
    }
                                ...
                                <form class="row g-3" method="POST" enctype="multipart/form-data">
									<div class="col-md-12">
                                        ...
                                        <select class="form-control form-select" id="category" name="category" required>
											<option vlaue="<?= $cate['category'] ?>"><?= $cate['category'] ?></option>
											<optgroup label="Classes">
                                                <?php foreach ($list as $value) { if ($value['page'] === 'class') { ?>
                                                    <option value="<?= $value['title'] ?>"><?= $value['title'] ?></option>
                                                <?php } } ?>
                                            </optgroup>
                                            <optgroup label="Courses">
                                                <?php foreach ($list as $value) { if ($value['page'] === 'course') { ?>
                                                    <option value="<?= $value['title'] ?>"><?= $value['title'] ?></option>
                                                <?php } } ?>
                                            </optgroup>
                                        </select>
